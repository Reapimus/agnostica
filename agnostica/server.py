...
class Server(Hashable):
    """Represents a server (or "guild") on a platform.
    ...
    def __init__(self, *, state, _platform, data, member_count: Optional[int] = None):
        self._state = state
    ...
    def vanity_url(self) -> Optional[str]:
        ...
        if self._platform.get_server_vanity_url is not None:
            return self._platform.get_server_vanity_url(self)
        return None
    ...
    async def leave(self):
        ...
        Leave the server.
        """
        await self._platform.kick_member(self.id, '@me')
    ...
    async def create_category(
        ...
        *,
        ...
        data = await self._platform.create_category(
            self.id,
            name=name,
            group_id=group.id if group is not None else None,
        )
        return Category(state=self._state, data=data, group=group, server=self)
    ...
    async def _create_channel(
        ...
        *,
        ...
        data = await self._platform.create_server_channel(
            self.id,
            content_type.value,
            name=name,
            topic=topic,
        ...
        channel = self._state.create_channel(data=data, group=group, server=self)
        return channel
    ...
    async def fetch_category(self, category_id: int, /) -> Category:
        ...
        data = await self._platform.get_category(self.id, category_id)
        return Category(state=self._state, data=data, server=self)
    ...
    async def fetch_channel(self, channel_id: str, /) -> ServerChannel:
        ...
        data = await self._platform.get_channel(self.id, channel_id)
        if data['serverId'] != self.id:
            raise InvalidData('The target channel does not belong to the current server.')

        channel = self._state.create_channel(data=data, group=None, server=self)
        return channel
    ...
    async def fetch_members(self) -> List[Member]:
        ...
        data = await self._platform.get_members(self.id)
        data = data
    ...
    async def bulk_award_member_xp(self, amount: int, *members: Member) -> Dict[str, int]:
        ...
        data = await self._platform.bulk_award_member_xp(self.id, [m.id for m in members], amount)
    ...
    async def bulk_set_member_xp(self, total: int, *members: Member) -> Dict[str, int]:
        ...
        data = await self._platform.bulk_set_member_xp(self.id, [m.id for m in members], total)
        totals: Dict[str, int] = data
        return totals
    ...
    async def create_role(
        ...
        *,
        ...
        data = await self._platform.create_role(
            self.id,
            payload=payload
        )

        role = Role(state=self._state, data=data)
        return role
    ...
    async def fetch_roles(self) -> List[Role]:
        ...
        """
        data = await self._platform.get_roles(self.id)
        return [Role(state=self._state, data=role_data) for role_data in data]

    async def fetch_role(self, role_id: str, /) -> Role:
        """|coro|
        ...
        data = await self._platform.get_role(self.id, role_id)
        return Role(state=self._state, data=data)

    async def getch_role(self, role_id: str, /) -> Role:
    ...
    async def ban(
        ...
        *,
        ...
        data = await self._platform.ban_server_member(self.id, user.id, reason=reason)
        ban = MemberBan(state=self._state, data=data, server=self)
        return ban
    ...
    async def unban(self, user: User):
        ...
        user: :class:`abc.User`
            The user to unban.
        """
        await self._platform.unban_server_member(self.id, user.id)
    ...
    async def fetch_ban(self, user: User) -> MemberBan:
        ...
        """
        data = await self._platform.get_server_ban(self.id, user.id)
        return MemberBan(state=self._state, data=data, server=self)
    ...
    async def bans(self) -> List[MemberBan]:
        ...
        data = await self._platform.get_server_bans(self.id)
        data = data
    ...
    async def kick(self, user: User):
        ...
        user: :class:`abc.User`
            The user to kick.
        """
        await self._platform.kick_member(self.id, user.id)
    ...
    async def create_webhook(
        ...
        *,
        ...
        data = await self._platform.create_webhook(
            self.id,
            name=name,
    ...
    async def fetch_webhook(self, webhook_id: str, /) -> Webhook:
        ...
        data = await self._platform.get_server_webhook(self.id, webhook_id)
    ...
    async def webhooks(self, *, channel: Optional[Union[ChatChannel, ListChannel]] = None) -> List[Webhook]:
        ...
        data = await self._platform.get_server_webhooks(self.id, channel.id if channel else None)
        return [
    ...
    async def create_group(
        ...
        *,
        ...
        data = await self._platform.create_group(
            self.id,
            payload=payload
        )

        group = Group(state=self._state, data=data, server=self)
        return group
    ...
    async def fetch_group(self, group_id: str, /) -> Group:
        ...
        data = await self._platform.get_group(self.id, group_id)
        group = Group(state=self._state, data=data, server=self)
        return group
    ...
    async def fetch_groups(self) -> List[Group]:
        ...
        data = await self._platform.get_groups(self.id)
        groups = [Group(state=self._state, data=group_data, server=self) for group_data in data]
        return groups
    ...
    async def fetch_subscription_tier(self, tier_type: ServerSubscriptionTierType, /) -> ServerSubscriptionTier:
        ...
        data = await self._platform.get_subscription_tier(self.id, tier_type.value)
        return ServerSubscriptionTier(state=self._state, data=data)
    ...
    async def fetch_subscription_tiers(self) -> List[ServerSubscriptionTier]:
        ...
        data = await self._platform.get_subscription_tiers(self.id)
        return [ServerSubscriptionTier(state=self._state, data=tier_data) for tier_data in data]
